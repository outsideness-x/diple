...
## 5. Что отгружено (2026-09-02)

Все одиннадцать правок в `main`, по коммиту на каждую, 145 юнит-тестов проходят. Ниже —
**расхождения с планом выше**: там, где реализация оказалась другой, права реализация.

| # | Коммит | Расхождение с планом |
|---|---|---|
| P1 | `Give the accent a second value for when it is ink` | brass-чернила `#7F6329`, а не `#8A6D2F`: посчитанный вариант давал 4.47:1 против нового тёплого холста — на волос ниже нормы. 57 call-sites. |
| P9 | `Make the light appearance paper rather than a screen` | без расхождений. |
| P7 | `Say "this one" the same way everywhere` | добавлен токен `DipleStroke.selection` (1.5). Свотчи тем читалки остались с ручным кольцом — `accentSoft` под ними показал бы бумагу, которой не будет; это записано в коде. |
| P10 | `Give the app one voice to speak in` | пришлось поправить UI-тест на `iCloud sync`. |
| P11 | `Pay off what the redesign kept tripping over` | без расхождений. |
| P3 | `Give the app a voice in type` | **четыре** роли, а не пять: заголовок заметки остался SF — заметка это то, что написал читатель, и там чаще всего смешаны письменности. Кегли не те, что в плане: `editorialLead` 24 (не 26), `editorialQuote` 17 (не 19) — Literata набирает на 7.5% шире при равной высоте прописных, замерено CoreText. Курсив не поставляется. |
| P2 | `Build one head for the three places…` | сюда же уехал пункт 6.1 (три кнопки захвата → меню `+`): масthead и есть то место, куда они переезжают. `DipleSearchField` создан здесь, а применён в P4/P5. |
| P4 | `Let the library be a shelf before it is a control panel` | **первая обложка 270 pt, а не ~190.** План считал шапку бесплатной, а она дороже навбара, который заменила. Чтобы не потерять ещё больше, поле поиска сделано **вызываемым** из заголовка полки — этого в плане не было. |
| P5 | `Take the plate off the board` | поиск тоже вызываемый, для симметрии с полкой; сортировка и раскладка слиты в один контрол. Первая заметка 232 pt (было 294). |
| P6 | `End the front page instead of letting it stop` | колофон **только на Home**: у полки и доски последняя строка уже рисует свою линейку, а счёт стоит в шапке — второй внизу был бы тавтологией. |
| P8 | `Let the page speak and the chrome keep quiet` | найден и починен **баг, которого в плане не было**: `ReaderChrome` для Carbon/Ink брал динамический `DipleColor.textPrimary`, поэтому при светлом интерфейсе и тёмной странице чернила хрома становились почти чёрными на тёмном баре. Пункт про верхнее поле страницы **снят**: `navigatorContentInset` уже отдаёт `DipleSpace.xl`, проверено на скриншоте. |

**Замеры до и после** (экран 402 × 874 pt, первый пиксель содержимого):

| Экран | Было | Стало |
|---|---|---|
| Library | 326 pt (37 %) | **270 pt (31 %)** |
| Notes | 294 pt (34 %) | **232 pt (27 %)** |
| Home | 258 pt (30 %) | **215 pt (25 %)** |

**Проверено измерением, а не на глаз:** контраст всех пяти акцентов по WCAG; ширина набора и
высоты прописных/строчных Literata против SF (CoreText); резолвится ли трейт веса на переменном
шрифте; соблюдает ли WebKit `-webkit-hyphenate-limit-*` (WKWebView, замер высоты слова).
